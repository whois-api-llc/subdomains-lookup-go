...
const (
	...
	userAgent      = "subdomains-lookup-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultSubdomainsLookupURL is the default Subdomains Lookup API URL.
const defaultSubdomainsLookupURL = `https://subdomains.whoisxmlapi.com/api/v1`
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.SubdomainsLookupBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultSubdomainsLookupURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.SubdomainsLookup = &subdomainsLookupServiceOp{client: client, baseURL: apiBaseURL}
